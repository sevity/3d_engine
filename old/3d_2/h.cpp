...
sf::Color calculateColor(const sf::Vector3f& normal, const sf::Color& baseColor) {
    ...
    float lightIntensity = std::max(0.0f, normal.z*10);  // Assuming light direction is (0, 0, -1)
    
    return sf::Color(baseColor.r * lightIntensity, baseColor.g * lightIntensity, baseColor.b * lightIntensity);
}
