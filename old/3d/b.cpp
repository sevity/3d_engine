...
int main() {
    ...
    while (window.isOpen()) {
        ...
        for (auto &point : points) {
            point = point - center;  // 중심점을 빼서 회전을 시작
            point = point.rotateY(5.10f);
            point = point + center;  // 중심점을 다시 더함
        }
        for (auto &edge : edges) {
            sf::Vector2f p1 = project(points[edge[0]]);
            sf::Vector2f p2 = project(points[edge[1]]);
            sf::Vertex line[] = {
